...
export interface CurrencyVerdict {
  ...
  demo: boolean;
  wire: CurrencyOnWire;
  /** Канонический ISO-код, либо `null` — кода нет (это только демо-сессия). */
  code: string | null;
...
/**
 * Валюта сессии, у которой платформа её не назвала.
 *
 * Поле пропадает из ответа именно у неё: сериализатор выбрасывает значения,
 * равные дефолтному. Проверяется наблюдением — на любой явно заданной валюте
 * поле приезжает и всё работает, ломалось только на этой.
 */
 ...
 * | ключа нет | `absent` | `false` (код → `USD`) | да |
 * | `""`, не строка | `invalid` | `false` (код → `USD`) | да |
 *
 ...
 *
 * Гипотезу выбрал владелец интеграции, у которого есть то, чего нет в ответе
 * платформы, — её настройка: **валюта не указана значит USD**, реальные
 * деньги. Так и читаем, и код валюты подставляем сам, чтобы игре было чем
 * форматировать деньги: без него она осталась бы вообще без валюты.
 *
 * Цена ошибки при этом всё равно асимметрична, и трактовка выбрана в
 * безопасную сторону:
 *
 ...
 *
 * Отклонение от доки остаётся отклонением: `deviates` доводит его до лога.
 */
export function classifyCurrency(info: SessionInfoResponse): CurrencyVerdict {
  ...
  return { demo: false, wire, code: DEFAULT_CURRENCY, deviates: true };
}
