...
| Auto `/wallet/end-round` when the final segment is delivered (incl. settling a round resumed on `authenticate`) for non-zero-payout rounds | |
...
When `authenticate` returns an active round (page refresh mid-round), the bridge **reopens** it — it materialises the segment cursor from `resumeFrom` but does **not** finalise on its own. The game drives it to completion by sending continuation `play({ roundId })` requests, exactly as in the live flow; the final segment then settles via `/wallet/end-round` and credits the win.

This includes single-segment rounds whose cursor resumes already at the final segment (e.g. a one-event winning base spin): the next continuation `play` re-delivers that final segment and settles it. (Earlier versions surfaced `NO_ACTIVE_SESSION` here, leaving the round open and the win uncredited.) `end-round` is idempotent, so a duplicated continuation won't double-settle.
