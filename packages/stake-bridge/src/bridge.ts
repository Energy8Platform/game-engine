...
export class StakeBridge {
  ...
  private async bootReplay(): Promise<RGSAuthenticateResponse> {
    ...
    // payoutMultiplier feeds the replay modal. Stake's /bet/replay returns payoutMultiplier
    // ALREADY as the ×bet multiplier (e.g. 0.3 for a 0.3× round) — the SAME scale
    // /wallet/play uses for round.payoutMultiplier — so surface it verbatim. (Dividing by 100
    // here made the replay modal show 0.003× for a 0.3× round: every win 100× too small.)
  ...
  private async startReplayRound(payload: PlayRequestPayload, requestId?: string): Promise<void> {
    ...
    if (!this.replayBook) {
      ...
      // payoutMultiplier from /bet/replay is the ×bet multiplier already — surface verbatim
      // (see bootReplay). No /100: that made every replay win render 100× too small.
      this.replayPayout =
  ...
  private async streamNextSegment(payload: PlayRequestPayload, requestId?: string): Promise<void> {
    ...
    if (next >= round.segments.length) {
      // No further segment to stream. If this is a round we resumed on
      // `authenticate` whose cursor is already pinned to the final segment
      // (e.g. a 1-event winning base round) RGS still considers it open and
      // EndRound was never called — `deliverSegment` was bypassed during
      // resume. Re-deliver the final segment so it settles via /wallet/end-round
      // rather than wedging the session with NO_ACTIVE_SESSION.
      if (round.rgsActive && !round.endRoundCalled) {
        await this.deliverSegment(round.cursor, requestId);
        return;
      }
      // Genuinely exhausted (already settled, or a replay round). Surface
