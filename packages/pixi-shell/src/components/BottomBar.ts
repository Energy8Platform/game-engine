...
function readout(host: ShellHost, label: string, value: string, opts: { valueSize?: number; align?: 'left' | 'center' | 'right'; fixedWidth?: number; color?: string; muted?: string } = {}): Readout {
  return new Readout({
    label: host.t(label),
    value,
...
export class BottomBar extends Container {
  ...
  private buildWide(): void {
    ...
    left.add(new IconButton('menu', { size: 36, glyph: 30, color: '#ffffff', hover: tokens.accent, onTap: () => this.host.openMenu() }));
    if (!state.replay) {
  ...
  private buildRightWide(isBase: boolean, showFsBlocks: boolean): FlexBox {
    ...
    if (isBase) {
      ...
      bet.cursor = 'pointer';
      bet.on('pointertap', () => { if (!this.betLocked()) this.host.openBetPicker(); });
    }
    ...
    if (isBase) {
      this.spin = new SpinDisc({ size: SPIN, glyph: 65, tokens, ticker: this.host.ticker, onSpin: () => this.host.emit('spin'), onStop: () => this.stopAutoplay() });
  ...
  private buildBuy(size: number, fontSize: number, border: number): BuyBonusBadge | null {
    ...
    if (feature) {
      ...
      return new BuyBonusBadge({ size, fontSize, border, bg: accent, fg: contrastText(accent), label: this.host.t('DISABLE'), tokens, ticker: this.host.ticker, onTap: () => this.host.deactivateFeature() });
    }
    return new BuyBonusBadge({ size, border, bg: tokens.accent, icon: 'ticket', iconSize: size * 0.55, iconColor: tokens.btnInk, label: '', tokens, ticker: this.host.ticker, onTap: () => this.host.openBuyBonus() });
  }
  ...
  private buildMobile(): void {
    ...
    const menu = new IconButton('menu', { size: 40, glyph: 26, color: '#ffffff', hover: tokens.accent, onTap: () => this.host.openMenu() });
    ...
    this.betReadout = bet;
    if (isBase) { bet.eventMode = 'static'; bet.cursor = 'pointer'; bet.on('pointertap', () => { if (!this.betLocked()) this.host.openBetPicker(); }); }
    betGroup.add(bet);
  ...
  private onBet(dir: 1 | -1): void {
    ...
    if (next !== state.bet) { state.bet = next; this.host.emit('betChange', next); this.host.render(); }
  }
  private onTurbo(): void {
    const { state, config } = this.host;
    const next = nextTurbo(state.turbo, config.features.turbo);
    state.turbo = next; this.host.emit('turboChange', next); this.host.render();
  }
  ...
  private stopAutoplay(): void {
    ...
    this.host.emit('autoplayStop'); this.host.render();
  }
