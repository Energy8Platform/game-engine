...
const SVGS: Record<string, string> = {
  ...
  // Lightning bolt(s) — escalating turbo level (1/2/3 overlapping bolts). The DOM uses a <g transform>
  // to scale/offset a 216×512 bolt into 24×24; Pixi's SVG parser ignores transforms, so the transforms
  // are BAKED into these coordinates (see scratchpad/bake-svg.mjs). Kept 1:1 with the DOM glyph.
  turbo1: `<g fill="currentColor"><path d="M12.47 0L6.94 10.88L10.97 10.88L8.25 24L17.07 10.31L12.85 10.31Z"/></g>`,
  turbo2: `<g fill="currentColor"><path d="M9.34 0L3.81 10.88L7.84 10.88L5.12 24L13.94 10.31L9.72 10.31Z"/><path d="M15.59 0L10.06 10.88L14.09 10.88L11.37 24L20.18 10.31L15.97 10.31Z"/></g>`,
  turbo3: `<g fill="currentColor"><path d="M6.22 0L0.69 10.88L4.72 10.88L2 24L10.81 10.31L6.6 10.31Z"/><path d="M12.47 0L6.94 10.88L10.97 10.88L8.25 24L17.06 10.31L12.85 10.31Z"/><path d="M18.72 0L13.19 10.88L17.22 10.88L14.5 24L23.31 10.31L19.1 10.31Z"/></g>`,
  ...
  // bonus ticket — perforated ticket (stroke) + bolt (fill), tilted −15°. DOM uses nested <g transform>;
  // baked into coords here for Pixi (see scratchpad/bake-svg.mjs). Stroke-width baked too (52×0.0354).
...
/** A standalone, parseable SVG document for an icon with `currentColor` resolved to `color`.
 *  Used by the Pixi renderer to build vector geometry via GraphicsContext.svg(). The 24×24
 *  viewBox is identical to the DOM glyph, so the Pixi icon is the same shape pixel-for-pixel. */
