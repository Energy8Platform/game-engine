...
export function optimizeLookupTable(
  ...
  for (let iter = 0; iter < maxIterations; iter++) {
    ...
    // NNLS path doesn't auto-enforce any of the verif.md risk limits —
    // those live in the tier-based path. Set the corresponding flags to
    // `true` so a caller using the NNLS algorithm always sees "satisfied"
    // for them (matches the pre-feature behaviour).
    const toleranceMet: ToleranceMet = {
      ...
      etlP10000: true,
      baseStdRange: true,
      payoutMultiplierCap: true,
      costMultiplierCap: true,
    };
