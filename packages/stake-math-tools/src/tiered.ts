...
export function buildTieredLookup(
  ...
  paramsIn: OptimizeParams,
): OptimizeResult {
  // starRating populates verif.md defaults for the whole risk suite.
  // Resolve it ONCE at the top into explicit per-limit values so every
  // downstream check (retry loop, tolerance object, recursive calls) sees
  // the same numbers — otherwise `--star-rating 2` would gate the failure
  // but never trigger the maxProb retry, because the retry condition
  // reads the raw field.
  const params: OptimizeParams = ((): OptimizeParams => {
    const sr = paramsIn.starRating;
    if (sr !== 2 && sr !== 3) return paramsIn;
    const d =
      sr === 3
        ? {
        ...
        : {
            ...
            maxCostMultiplier: 1_000,
          };
    return {
      ...paramsIn,
      maxProb5K: paramsIn.maxProb5K ?? d.maxProb5K,
      maxProb10K: paramsIn.maxProb10K ?? d.maxProb10K,
      maxCVaRNormalized: paramsIn.maxCVaRNormalized ?? d.maxCVaRNormalized,
      maxEtl40xCost: paramsIn.maxEtl40xCost ?? d.maxEtl40xCost,
      maxEtlP10000: paramsIn.maxEtlP10000 ?? d.maxEtlP10000,
      minBaseStd: paramsIn.minBaseStd ?? d.minBaseStd,
      maxBaseStd: paramsIn.maxBaseStd ?? d.maxBaseStd,
      maxPayoutMultiplier: paramsIn.maxPayoutMultiplier ?? d.maxPayoutMultiplier,
      maxCostMultiplier: paramsIn.maxCostMultiplier ?? d.maxCostMultiplier,
      starRating: undefined, // already compiled into explicit limits above
    };
  })();

  // maxProb5K / maxProb10K: Stake's "Within Probability Limits" check
  // enforced by auto-tuning largeTarget. P(K+) = (rows with payout ≥ K) / T,
  // and the rows ≥ K live in the cap+large tier. Shrinking largeTarget
  // shrinks the count there in direct proportion, so a single ratio scaling
  // closes most overshoots in one retry. Loop a few times because:
  //  - T also changes when the high-tier count changes (W rebalances),
  //  - shape decay further reshapes the per-bucket counts,
  // so the second pass usually nails the limit. Disables itself on the
  // recursive call via maxProb5K/10K=undefined.
  if (params.maxProb5K !== undefined || params.maxProb10K !== undefined) {
    const cachedRows: LookupRow[] = [];
    for (const r of rowsIn) cachedRows.push(r);

    // If user gave no largePmThreshold, default it so the subsampled tier
    // exactly covers the constrained zone (rows ≥ 5K / 10K bet multiples).
    // Use the smaller threshold so subsample reaches across both.
    let effLargePm = params.largePmThreshold;
    if (effLargePm === undefined) {
      effLargePm = Math.min(
        params.maxProb5K !== undefined ? 5000 : Infinity,
        params.maxProb10K !== undefined ? 10000 : Infinity,
      );
      if (!Number.isFinite(effLargePm)) effLargePm = undefined as unknown as number;
    }
    // Initial largeTarget: respect the caller's setting; otherwise start at
    // the tightest active limit so the first pass is already near the mark.
    let effLargeTarget = params.largeTarget;
    if (effLargeTarget === undefined) {
      const minLimit = Math.min(
        params.maxProb5K ?? Infinity,
        params.maxProb10K ?? Infinity,
      );
      effLargeTarget = Number.isFinite(minLimit) ? minLimit : 0.01;
    }
    ...
    for (let iter = 0; iter < 4; iter++) {
      const inner: OptimizeParams = {
        ...params,
        largeTarget: effLargeTarget,
        largePmThreshold: effLargePm,
        maxProb5K: undefined,
        maxProb10K: undefined,
      };
      ...
      trail.push({ largeTarget: effLargeTarget, p5KScaled, p10KScaled });
      const v5 = params.maxProb5K !== undefined ? p5KScaled / params.maxProb5K : 0;
      const v10 = params.maxProb10K !== undefined ? p10KScaled / params.maxProb10K : 0;
      const worst = Math.max(v5, v10);
      if (worst <= 1) break;
      effLargeTarget = Math.max(1e-7, effLargeTarget / worst);
    }
    const finalResult = lastResult!;
    const last = trail[trail.length - 1];
    finalResult.toleranceMet.prob5K =
      params.maxProb5K === undefined || last.p5KScaled <= params.maxProb5K;
    finalResult.toleranceMet.prob10K =
      params.maxProb10K === undefined || last.p10KScaled <= params.maxProb10K;
    if (!finalResult.toleranceMet.prob5K || !finalResult.toleranceMet.prob10K) {
      finalResult.warnings.push(
        `Within Probability Limits not met after ${trail.length} retries: ` +
          `P(5K)_scaled=${last.p5KScaled.toFixed(5)} (limit ${params.maxProb5K ?? 'n/a'}), ` +
          `P(10K)_scaled=${last.p10KScaled.toFixed(5)} (limit ${params.maxProb10K ?? 'n/a'}). ` +
          `Tried largeTarget=${trail.map((t) => t.largeTarget.toExponential(2)).join(',')}.`,
      );
    } else if (trail.length > 1) {
      finalResult.warnings.push(
        `Within Probability Limits met after ${trail.length} retries: ` +
          `P(5K)_scaled=${last.p5KScaled.toFixed(5)}/${params.maxProb5K ?? 'n/a'}, ` +
          `P(10K)_scaled=${last.p10KScaled.toFixed(5)}/${params.maxProb10K ?? 'n/a'} ` +
          `(largeTarget converged to ${effLargeTarget.toExponential(2)}).`,
      );
    }
    return finalResult;
  }
  ...
  if (outCap.length > params.nRowsOut) {
    // Too many cap rows — keep highest-payout (cap rows already cluster
    // inside [0.95 × max, max], so top-N here is essentially uniform across
    // that narrow band).
    ...
    // Cap fits, but cap+large overflow the LUT. Two failure modes if we
    // just slice top-N here:
    //   1. Top-N by payout systematically over-weights the high end → mean
    //      (and therefore RTP) blows past target before refinement gets to
    //      run.
    //   2. When `largePmThreshold` is low enough that the whole pool
    //      lands in `srcLarge` (typical for BONUS modes with cost ≥ 100×),
    //      `srcSmall` is empty and the small-tier RTP refinement scope is
    //      skipped entirely. The large subsample IS the output's RTP — it
    //      has to be RTP-aware here or nothing else will pull it.
    // Fix: use `rtpAwareSampleNonZero` to pick large rows whose mean lands
    // at exactly what's needed for the achieved RTP to hit `targetRTP`,
    // given the cap rows' fixed contribution.
    const allowedLarge = params.nRowsOut - outCap.length;
    if (allowedLarge > 0) {
      const capSum = outCap.reduce((s, r) => s + r.payoutCents, 0);
      const T = outCap.length + allowedLarge; // weight=1 across the high tier
      const targetTotalSum = params.targetRTP * T * betCost;
      const targetSumLarge = targetTotalSum - capSum;
      const targetMeanLarge = allowedLarge > 0 ? targetSumLarge / allowedLarge : 0;
      if (targetMeanLarge > 0) {
        const sampleRes = rtpAwareSampleNonZero(
          srcLarge,
          allowedLarge,
          targetMeanLarge,
          50,
        ...
        outLarge = sampleRes.sampled;
      } else {
        // Target mean ≤ 0: cap alone already at/over RTP target. Fall back
        // to a stratified sample so we at least preserve the shape; the
        // RTP overshoot will be flagged by `toleranceMet.rtp`.
        outLarge = stratifiedSmallSampleNonZero(srcLarge, allowedLarge, 50, seed + 31);
      }
    } else {
      outLarge = [];
    }
  ...
  // Compute Stake report early so we can fold prob5K/prob10K/CVaR/ETL
  // into the tolerance object below.
  const costMultiplierEff = params.costMultiplier ?? 1;
  const stakeReportEarly = computeStakeReport(outRows, achieved, betCost, costMultiplierEff);

  // starRating defaults are compiled into explicit params at the top of
  // this function, so we read them straight off `params` here. Each is
  // `undefined` only when neither the caller nor a starRating supplied
  // it — that's the "skip this rule" signal.
  const lim = {
    maxProb5K: params.maxProb5K,
    maxProb10K: params.maxProb10K,
    maxCVaRNormalized: params.maxCVaRNormalized,
    maxEtl40xCost: params.maxEtl40xCost,
    maxEtlP10000: params.maxEtlP10000,
    minBaseStd: params.minBaseStd,
    maxBaseStd: params.maxBaseStd,
    maxPayoutMultiplier: params.maxPayoutMultiplier,
    maxCostMultiplier: params.maxCostMultiplier,
  };

  const baseStdCheckable = Math.abs(costMultiplierEff - 1) < 1e-9;
  const baseStdInRange =
    !baseStdCheckable ||
    ((lim.minBaseStd === undefined || stakeReportEarly.baseStd >= lim.minBaseStd) &&
      (lim.maxBaseStd === undefined || stakeReportEarly.baseStd <= lim.maxBaseStd));
  ...
  const toleranceMet: ToleranceMet = {
    ...
    prob5K:
      lim.maxProb5K === undefined ||
      stakeReportEarly.prob5KScaled <= lim.maxProb5K,
    prob10K:
      lim.maxProb10K === undefined ||
      stakeReportEarly.prob10KScaled <= lim.maxProb10K,
    cvar:
      lim.maxCVaRNormalized === undefined ||
      stakeReportEarly.cvarNormalized <= lim.maxCVaRNormalized,
    etl40xCost:
      lim.maxEtl40xCost === undefined ||
      stakeReportEarly.etl40xCost <= lim.maxEtl40xCost,
    etlP10000:
      lim.maxEtlP10000 === undefined ||
      stakeReportEarly.etlP10000 <= lim.maxEtlP10000,
    baseStdRange: baseStdInRange,
    payoutMultiplierCap:
      lim.maxPayoutMultiplier === undefined ||
      stakeReportEarly.payoutMultMax <= lim.maxPayoutMultiplier,
    costMultiplierCap:
      lim.maxCostMultiplier === undefined ||
      costMultiplierEff <= lim.maxCostMultiplier,
  };
  ...
  // Stake report (already computed above for the tolerance check).
  const stakeReport = stakeReportEarly;
