...
export class CasinoGameSDK {
  ...
  /**
   * `true` when the game was launched as a historical-round replay
   * (`config.replayMode`). Convenience over `sdk.config?.replayMode`.
   *
   * In replay mode the game should hide the balance / bet selector /
   * autoplay / buy-bonus UI, surface only a "Play / Play Again" CTA, and
   * render `config.disclaimerLines`. Returns `false` before `ready()`.
   */
  get isReplay(): boolean {
    return this._config?.replayMode === true;
  }
