...
| `sdk.isReplay` | `boolean` | `true` when launched as a historical-round replay (sugar over `config.replayMode`) |

## Replay mode

A game can be launched to **replay a historical round** (a previously-played
round re-watched move-by-move) instead of placing real bets. The same
...
The host sets `config.replayMode = true` in `INIT`. The game reacts via the
`sdk.isReplay` convenience getter:
...
if (sdk.isReplay) {
  ...
  renderDisclaimer(config.disclaimerLines);
}

// Spins work exactly as in live play — the host feeds recorded results back.
const result = await sdk.play({ action: 'spin', bet: config.betLevels?.[0] ?? 0 });
```

Behaviour is identical whether the game runs under Stake or the Energy8 host —
it only reads one flag.

### Host side (recipe — built on `Bridge`, nothing new)

The host is a thin proxy (`PLAY_REQUEST → PLAY_RESULT`). To replay a round it
swaps the data source and flips the flag:

1. **Detect a replay launch** from your own URL parameter, e.g.
   `?replay=1&roundId=…`.
2. **Fetch the recorded round once** — the whole round as an ordered list of the
   same `PlayResultPayload` objects live play returns:
   `GET /games/{gameId}/replay/{roundId} → { results: PlayResultPayload[] }`.
3. On **`GAME_READY`** send `INIT` with `config.replayMode = true`; take
   balance/currency from the recorded results; `session: null` (or restored).
4. On **`PLAY_REQUEST`** serve `results[cursor]` and increment the cursor — do
   **not** call the wallet. The first `spin` after the list is exhausted resets
   `cursor = 0` ("Play Again").
5. On **`GET_STATE`** return a snapshot at the current cursor; on **`GET_BALANCE`**
   return the recorded ("historical") balance.

```ts
// inside the host's existing Bridge wiring, when launched as a replay
let cursor = 0;
const { results } = await fetchRecordedRound(gameId, roundId);

bridge.on('GAME_READY', (_p, id) => {
  bridge.send('INIT', {
    balance: results[0]?.balanceAfter ?? 0,
    currency: results[0]?.currency ?? 'USD',
    config: { id: gameId, type: 'slot', replayMode: true, /* … */ },
    session: null,
  }, id);
});

bridge.on('PLAY_REQUEST', (payload, id) => {
  if (payload.action === 'spin' && cursor >= results.length) cursor = 0; // Play Again
  const next = results[cursor++];
  if (!next) {
    bridge.send('PLAY_ERROR', { code: 'NO_ACTIVE_SESSION', message: 'Replay exhausted' }, id);
    return;
  }
  bridge.send('PLAY_RESULT', next, id);
});
```
