...
describe('semver survives hostile input', () => {
  ...
  it('accepts leading-zero versions (regex does not reject them)', () => {
    expect(parseVersion('01.02.03')).toEqual([1, 2, 3]);
    expect(satisfies('01.02.03', '^1.0.0')).toBe(true);
  });
  ...
  it('rejects a huge but well-formed version rather than losing precision silently', () => {
    // Document whichever behaviour is real — assert what actually happens, do not force a result.
    const huge = '999999999999999999999.0.0';
    expect(() => parseVersion(huge)).not.toThrow();
  });
});
