...
function plaqueReadout(host: PixiComponentContext, label: string, value: string): Readout {
  return new Readout({
    label: host.t(label),
    value,
...
export class BottomBar extends Container {
  ...
  private buildMobile(): void {
    ...
    const menuBtn = new IconButton('menu', { color: '#fff', hover: tokens.accent, onTap: () => this.host.actions.openMenu() });
...
class WinPill extends Container {
  ...
  constructor(host: PixiComponentContext, label: string, value: string) {
    super();
    this.host = host;
...
function mobileReadout(host: PixiComponentContext, label: string, value: string): Readout {
...
function buyBadgeSize(_host: PixiComponentContext): number {
  return 80;
}
