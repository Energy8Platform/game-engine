...
describe('formatCurrency', () => {
  ...
  // Regression: a player read "2,50" as 250. The decimal separator must be '.' by default —
  // the platform convention — so a fraction can never be mistaken for a thousands group.
  it('never uses a comma as the decimal separator by default', () => {
    expect(formatCurrency(2.5, eur)).toBe('€2.50');
    expect(formatCurrency(0.75, eur)).toBe('€0.75');
