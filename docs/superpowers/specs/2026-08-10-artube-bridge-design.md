...
{ "v": 1, "seed": {"server":"…","client":"…","nonce":42},
  "script": "sha256:…", "cursor": 3, "total_win_x": 12.5,
  "actions": [{"a":"pick","p":{"slot":2}}] }
```
...
влезает в лимит конверта 128 KB.

Обработка любого запроса — чистая функция:

1. взять `round_state` (из `SessionInfo.last_round` или ответа предыдущей RPC);
2. воспроизвести раунд в `e8-server` из сидов + лога интерактивных действий →
   полный список сегментов;
3. отдать сегмент `cursor`, отправить RPC с `cursor + 1`, забыть всё.

Под может умереть между любыми двумя шагами: следующий запрос сядет на другой под и
восстановится из `round_state`. `e8-server` — эфемерный калькулятор с `--sessions memory`
и коротким TTL: воспроизведение идёт под собственным эфемерным `round_id`, не связанным
с `round_id` от Artube, и после снятия сегментов раунд в движке никому не нужен.
...
Движок на entry-экшене играет раунд до конца, поэтому итог известен сразу.
...
3. **Стоимость воспроизведения** — один прогон раунда движком на каждый сегмент. Для
   слота с полусотней фриспинов это доли миллисекунды на Rust, но при кратном росте
   длины раунда стоит померить.
