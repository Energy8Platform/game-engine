...
```
Браузер (один хост игры — фронт и бэк за одним reverse proxy, по разным путям)
 │   ArtubeBridge ←── MemoryChannel ──→ CasinoGameSDK ← код игры не меняется
 │        │  WS  <хост>/api/<slug>/api/ws?sessionId=…  (same-origin, без CORS)
 ▼        ▼
...
> **Поправка (2026-08-14, редакция 2).** Первая редакция этой врезки тоже была
> неверна — привожу обе, потому что интересна как раз закономерность.
>
> Изначально в схеме стояло «один **домен** игры», и мост выводил `apiBase` как
> `url.origin` → `wss://host/api/ws`. Это `404`: в корне домена бэкенда нет.
> Поправка №1 (`0.1.1`) прочла это как «игра смонтирована под пер-игровым
> **префиксом пути**, и её `/api/**` живёт под тем же префиксом», и взяла
> каталог страницы → `wss://host/artube-o7df8qem5k/api/ws`. Тоже мимо, и хуже:
> на этом хосте любой путь `/<что-угодно>/api/*` отвечает на апгрейд `101` и
> дальше молчит, так что игра не падала с ошибкой, а висла без `init`.
>
> Измеренная форма живого стенда: статика игры — бакет CDN под `/<slug>/`,
> бэкенд — **отдельный маршрут прокси `/api/<slug>/**`**, и наш собственный
> `/api/ws` остаётся на конце (удвоенное `api` настоящее).
>
> ```
...
> ```
>
> Исправлено в `artube-bridge@0.1.2`: `apiBase` = путь страницы запуска,
> перевешенный под `/api`; в корне (дев) перевешивать нечего и остаётся origin.
> Подробности — `docs/artube-integration.md`, раздел «Статика и бэкенд стоят по
> разным путям одного хоста».
>
> **Мораль, которая дороже самой правки.** Обе неверные редакции выводились из
> правдоподобного чтения формулировки в доке; ни одна не была измерена. Дев
> здесь не проверяет ничего: игра стоит в корне, где все три правила совпадают.
> Единственный способ узнать форму — запрос к живому деплою.
