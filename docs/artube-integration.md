...
```
браузер                       один адрес игры                  платформа
┌──────────────────────┐ <префикс>/api/ws ┌─────────────────┐  wss  ┌──────────┐
...
- Фронт и бэкенд Artube отдаёт **по одному адресу игры**, разделяя по пути:
  `/api/**` → бэкенд, остальное → статика. CORS в этой схеме просто нет, а
  бэкенд на другом домене — неподдерживаемая конфигурация.

### Адрес игры — это не корень домена

Реальный деплой монтирует каждую игру под **пер-игровым префиксом пути**:

```
страница  https://dev.artube-888.live/artube-o7df8qem5k/?sessionId=…&gameId=artube-o7df8qem5k
сокет     wss://dev.artube-888.live/artube-o7df8qem5k/api/ws?sessionId=…
```

Поэтому `apiBase` мост берёт из **каталога страницы запуска** (origin + путь
по последний `/`), а не из origin: origin выбросил бы префикс и увёл сокет на
`wss://dev.artube-888.live/api/ws`, который не принадлежит никакой игре.
Каталог покрывает обе формы одним правилом — под пер-игровым поддоменом
...
Со стороны сервера симметрично: `artube-server` принимает свои `/api/**` и
голыми, и под префиксом, так что снимает ли reverse proxy платформы префикс
перед бэкендом — не наша забота.
...
**Чем дев отличается от прода — и почему это не мешает.** Форма одна и та же
(фронт и бэкенд по одному адресу, разделённые по пути `/api/**`), но адрес
разный: dev-сервер отдаёт игру в **корне** (`http://localhost:5173/`), поэтому
и прокси висит на голом `/api`; прод монтирует игру под **пер-игровым
префиксом пути**, где тот же маршрут — это `<префикс>/api`. Мост ничего из
этого не зашивает: он выводит адрес бэкенда из каталога страницы, на которой
запущен, — в деве это корень, в проде префикс. Одна сборка, оба случая.
Ровно на этом месте и был баг: пока `apiBase` брался из origin, дев работал,
а каждый настоящий деплой уходил на `/api/ws` в корне домена.
