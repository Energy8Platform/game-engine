...
// ─── Pixi app: a faux game canvas behind the shell ─────────────────────────
...
const bgLabel = new Text({
  text: 'GAME CANVAS',
  style: { fill: 'rgba(255,255,255,.18)', fontSize: 13, fontWeight: '700', letterSpacing: 2, fontFamily: 'sans-serif' },
});
bg.addChild(bgRect, bgLabel);
...
function drawBg(): void {
  ...
  bgRect.clear();
  bgRect.rect(0, 0, w, h).fill(currentTheme === 'light' ? '#e7ecf4' : '#140f2e');
  bgLabel.style.fill = currentTheme === 'light' ? 'rgba(15,23,42,.2)' : 'rgba(255,255,255,.18)';
  bgLabel.position.set((w - bgLabel.width) / 2, (h - bgLabel.height) / 2);
}
drawBg();

// ─── Create the shell (single source of truth = this demo's `state`) ───────
const shell: PixiGameShell = createPixiShell({
  app,
  ...
  gameInfo: {
    sections: [
      ...
      },
      {
        ...
        grid: { cols: 5, rows: 3 },
        shapes: [
          { name: 'V shape', cells: [[0, 0], [1, 1], [2, 2], [3, 1], [4, 0]], description: 'Symbols landing in a V across the reels.' },
          { name: 'Top row', cells: [[0, 0], [1, 0], [2, 0], [3, 0], [4, 0]], description: 'A full line across the top.' },
        ],
      },
...
function applyTheme(t: 'dark' | 'light'): void {
  currentTheme = t;
  shell.setTheme({ scheme: t, accent: 'red' });
  drawBg();
  log(`theme → ${t}`);
  renderToolbar();
}
...
// ─── Screen preset switching (resizes the Pixi canvas; shell follows) ──────
...
function renderToolbar(): void {
  toolbarEl.innerHTML = '';
  toolbarEl.appendChild(group('Screen', SCREENS.map((s) => chip(s.label, `${s.width}×${s.height}`, s.id === currentScreen.id, () => applyScreen(s)))));
  toolbarEl.appendChild(group('Bottom-bar mode', MODES.map((m) => chip(m.label, null, m.id === currentMode, () => applyMode(m.id)))));
  toolbarEl.appendChild(group('Theme', (['dark', 'light'] as const).map((t) => chip(t === 'dark' ? 'Dark' : 'Light', null, t === currentTheme, () => applyTheme(t)))));
  toolbarEl.appendChild(group('Language', [
    chip('English', null, !isSocial, () => applySocial(false)),
    chip('Social', null, isSocial, () => applySocial(true)),
  ]));
  toolbarEl.appendChild(group('Actions', [
...
if (params.get('kiosk')) {
  ...
  const fit = () => { gameEl.style.width = `${window.innerWidth}px`; gameEl.style.height = `${window.innerHeight}px`; gameEl.style.borderRadius = '0'; app.renderer.resize(window.innerWidth, window.innerHeight); drawBg(); };
  fit();
